...
void setup() {
  ...
  ADMUX  = ADC_CHANNEL | 0xC0; // Channel sel, right-adj, use AREF pin
...
ISR(ADC_vect) { // Audio-sampling interrupt
  ...
  int16_t MID = 512;
  capture[samplePos] =
    ((sample > (MID-noiseThreshold)) &&
     (sample < (MID+noiseThreshold))) ? 0 :
    sample - MID; // Sign-convert for FFT; -512 to +511
